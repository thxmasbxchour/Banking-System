...
class Form():
    ...
    def login_register(self):
        ...
        if q.lower() == "register".lower():
            ...
            mycustomers = Register("sortcode", "name", "valid_from", "valid_end", "account_num", "card_num", "cvv")
...
class Register():
    ...
    def __init__(self,sortcode,name,valid_from,valid_end,account_num, card_num, cvv): # Defines everything
        super().__init__()
    ...
    def get_name(self):
        name1 = input("What is the name that you want to be on your card > ") # Gets your name to be displayed and added to DB
    ...
    def sort_code(self):
        mylist = []
        for i in range(0,6):
            sort_number = random.choice(self.nums) # Randomly selects your digits from the previously defined variable
            mylist.append(sort_number)
        mylist.insert(2, "-") # Puts spaces between the certain numbers so it looks like a sort code
        mylist.insert(5, "-") 
    ...
    def account_number(self):
        mylist = []
        for i in range(0,8): 
            acct_num = random.choice(self.nums) # Selects random choice of numbers for account number
    ...
    def cvv_num(self):
        mylist = []
        for i in range(0,3):
            cvv_num = random.choice(self.nums) # Selects random choice of numbers for CVV (only 3)
    ...
    def card_number(self):
        mylist = []
        for i in range(0,16):
            card_number = random.choice(self.nums) # Selects random choice of numbers for card number
    ...
    def ValidFrom(self):
        ...
        print("Valid From: " + str(currentMonth) + "/" + str(currentYear)) # Puts year and month together
        return str(currentMonth) + "/" + str(currentYear)

    def ValidEnd(self):
        
        fiveyears = datetime.now() + relativedelta(months=60) # Gets 5 years into future
        month = fiveyears.month
        year = fiveyears.year

        print("Valid Till: " + str(month) + "/" + str(year)) # Puts year and month together
    ...
    def do_tasks_and_add_data_to_file(self):
        data_to_append = [ 
            [self.get_name(), self.card_number(), self.sort_code(), self.account_number(), self.cvv_num(), self.ValidFrom(), self.ValidEnd(), self.username(), self.password(), '0']
        ]
...
class HomePage(Login):
    ...
    def to_do(self):
        ...
        if q == "1": # Checks if "1" was written
            ...
            """)
            sort = input("What is the sort code of the person you would like to send money to > ") # Asks the user the sort code of the reciever
            acct_num = int(input("What is the account number of the person you would like to send money to > ")) # Asks the user the account number of the reciever
            amount = float(input("How much money would you like to send to them > ")) # Asks the user how much money they would like to send
            df = pd.read_csv("info.csv") # Opens and reads info.csv
            mask = (df['sortcode'] == sort) & (df['accountnum'] == acct_num) # Checks if the sort code and account number in the file = the one provided by the user
            if mask.any(): # Checks if the mask variable is True with the .any() command
                with open('info.csv', newline='') as csvfile: # Basically does the method balance()
                    reader = csv.reader(csvfile)
                    for row in reader:
                        if row[8] == self.pswrd:   
                            if row[7] == self.user:
                ...
                if (Balance >= amount):
                    df.loc[mask, 'balance'] += amount # Adds the amount the balance section
                    df.to_csv('info.csv', index=False) # Replaces temp file with original one
                    ...
                    if mask1.any(): # Checks if mask1 is True
                        df.loc[mask1, 'balance'] -= amount # Minuses the amount specified by the user from senders account
                        df.to_csv("info.csv", index=False) # Replaces temp file with original one

                        with open("info.csv", newline="") as csvfile: # Opens the csv file again
                            reader = csv.reader(csvfile) # Creates variable to read the csv file
                            for row in reader: # Iterates through all the rows in the csv file
                                    if str(acct_num) in row[3]: # Checks if the account number is in the correct row
                                        reciever = row[7] # Makes the username of the reciever into a variable
                                        
                                        date = datetime.now().strftime('%Y-%m-%d %H:%M:%S') # Gets the exact date and time
                                        append_info = [self.user, reciever,amount,date] # Gets the information we are going to add to our file
                                        with open('transactions.csv', 'a',newline="") as csvfile: # Opens the csv file we are storing our data in 
                                            writer_object = csv.writer(csvfile) # Makes a writer object so we can write into the file
                                            writer_object.writerow(append_info) # Writes the information to a new row in the file
                                            csvfile.close() # Closes the file so no data breaches can occur

                    done = input("Succesfully Completed Transaction. Press Enter to see balance or 'exit' to quit > ")
                    if done == "":
                        self.balance()
                        ...
                        self.balance()
                elif (Balance < amount): # Checks if your balance is bigger than amount
...
class ChangeInformation(Login):
    ...
    def change_username(self):
        ...
        if new_username.lower() in data:
            ...
            mask = (df['username'] == self.user) & (df['password'] == self.pswrd) # Checks if the username and pswrd in the file = the one provided by the user
            if mask.any():
                with open('info.csv', newline='') as csvfile: # Basically does the method balance()
                    reader = csv.reader(csvfile)
                    for row in reader:
                    ...
                    df.loc[mask, 'username'] = new_username # Adds the amount the balance section
                    df.to_csv('info.csv', index=False) # Replaces temp file with original one
    ...
    def change_password(self):
        ...
        if mask.any():
            with open('info.csv', newline='') as csvfile: # Basically does the method balance()
                    ...
                    df.loc[mask, 'password'] = new_pswrd # Adds the amount the balance section
                    df.to_csv('info.csv', index=False) # Replaces temp file with original one
